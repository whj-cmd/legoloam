...
void callbackNode(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    cout<<pc_curr.size()<<endl;
    // pcl::PointCloud<PointType> static_and_uncertain_points;
    
    // for(const PointType& point : pc_curr.points){
    //     if(point.intensity < 0.1f){

    //         continue;
    //     }

    //     static_and_uncertain_points.push_back(point);
    // }
    // cout<<"静态点和不确定点的数目为："<<static_and_uncertain_points.size()<<endl;
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber NodeSubscriber = nh.subscribe<sensor_msgs::PointCloud2>(pointCloudTopic2, 5000, callbackNode);
